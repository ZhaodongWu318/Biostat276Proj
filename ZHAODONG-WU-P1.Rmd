...
According to the result, the accuracy is 0.005 if we use a multivariate $t$ distribution with a high stanard deviation.
...
We can see simulation points (especially $x_2$) deviate much from our target distribution.
...
summary_Q7(MC.simMHC2$x1, MC.simMHC2.rb)
```
...
```
```{r}
summary_Q7(Q8$x1[Q8$Accept == "TRUE"], Q8$x2[Q8$Accept == "TRUE"])
```
...
$\textbf{(c)}$. Consider $p = 1$. Add a hyper prior on $\gamma = \frac{1}{\lambda^2} ∼ Gamma(a,rate = b)$. Assess how the marginal prior of $\beta$ changes for $a = 1$ and values of $b \ge 1$.
...
\begin{eqnarray}
p(\lambda^2 | Y, \beta, \sigma^2, \{ \tau_1^2, \cdots, \tau_p^2 \}) &\propto& (\lambda^2)^{a-1} \text{exp}(-b\lambda^2-\frac{1}{2}\sum_{j=1}^p \tau_j^2) \nonumber \\
&\sim& Gamma(a, b+\frac{1}{2}\sum_{j=1}^p \tau_j^2) \nonumber 
\end{eqnarray}
...
gibbs_2e <- function(init_2e,                     # initial values
                     ...
                     seed = 1998,
                     reg_path = FALSE, sen_analysis = FALSE){
  set.seed(seed)
  # initialization---------------------------------------------
  ...
  for (isim in 1:nsim1) {
    ...
    sigma_sq <- 1/(rgamma(1, shape = 0.1 + 442/2, 
                       rate = 0.5 * t(Y - X %*% t(beta)) %*% (Y - X %*% t(beta)) + 10))
    #lambda_sq
    if(reg_path == TRUE){
      lambda_sq <- lambda_sq
    }else if(sen_analysis == TRUE){
...
comparison <- tibble("MCMC-Gibbs" = coeff_med, 
                     "Glmnet" = coeff_2e[-1])
...
library(reshape2)
library(plotmo)
lambda_2f <- data.frame(lambda_sq =exp(seq(-5, 5, 0.1)))
...
results_plot_2f$lambda <- log(sqrt(lambda_2f$lambda_sq))
results_plot_2f <- melt(results_plot_2f, id.vars = names(results_plot_2f)[11])
```

```{r}
ggplot(results_plot_2f, aes(lambda, value, color = variable)) +
...
```{r}
ab_2g <- data.frame(a = c(0.5,1,5,10,15),
                    b = c(0.5,1,5,10,15))
...
results_2g <- lapply(test_2g, function(x){apply(x$beta, 2,
                                                function(beta){
                                                  quantile(beta,c(0.1, 0.5, 0.9))}
                                          )})
results_ci_2g <- data.frame(Reduce("rbind", results_2g))
colnames(results_ci_2g) <- colnames(dia2e_x)

df_compare <- matrix(NA, 10, 5)
for(i in 0:4){
  for(j in 1:10){
    df_compare[j, i + 1] <- paste0(round(results_ci_2g[3*i+2,j],2),"(",
                                   round(results_ci_2g[3*i+1,j],2), ", ",
                                   round(results_ci_2g[3*i+3,j],2), ")")
  }
}
```
