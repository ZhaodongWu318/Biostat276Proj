...
lasso_2e <- glmnet(dia2e_x, dia2e_y)
cvfit_2e <- cv.glmnet(dia2e_x, dia2e_y)
coef(cvfit_2e, s = "lambda.min")

cvfit_2e$lambda.min
plot(lasso_2e,label = TRUE)
```
...
$\textbf{(g)}$. Free $\lambda$ and carry out a sensitivity analysis assessing the behavior of the posterior distribution of $\beta$ and $\sigma^2$, as hyper parameters $a$ and $b$ are changed. Explain clearly the rationale you use to assess sensitivity and provide recom- mendations for the analysis of the `diabetes` data.